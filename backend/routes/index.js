...
// ---------------------------------------------------------
// AUTO CHECK LIMIT ORDERS EVERY 10 sec
// ✅ FIX: Guard with mongoose.connection.readyState check
// Without this, setInterval fires immediately on require() — before DB is connected
// causing: "Operation orders.find() buffering timed out after 10000ms"
// ---------------------------------------------------------
const mongoose = require("mongoose");

setInterval(async () => {
  // readyState 1 = connected. Skip if DB not ready yet.
  if (mongoose.connection.readyState !== 1) {
    console.warn("⚠️ Skipping order check — MongoDB not connected yet");
    return;
  }
  try {
    const currentPrices = await getLivePricesFromAPI();
